"""The vendored backend-contract copy cannot silently drift (there is otherwise
no cross-repo drift check on the vendored contract copy).

``scripts/sync-backend.sh`` (in VolView) regenerates the contract, copies
``fixtures/`` + ``generated/`` here, and writes ``MANIFEST.sha256`` over
every copied file. This suite re-hashes the vendored tree against that manifest, so BOTH
...
Re-running ``sync-backend.sh`` regenerates from the single source and rewrites the
...
# The subtrees sync-backend.sh copies (and hashes). The manifest lives at the
...
def test_manifest_present_and_nonempty():
    # The manifest is written by sync-backend.sh; its absence means the vendored
    # copy was placed WITHOUT the drift guard — fail closed.
    assert _MANIFEST_PATH.exists(), (
        "MANIFEST.sha256 missing; re-run backend-contract/scripts/sync-backend.sh"
    )
...
def test_every_manifest_entry_matches_the_vendored_file():
    ...
    for rel_path, expected_digest in entries.items():
        ...
        assert target.exists(), (
            "manifest lists a file that is not vendored: %s" % rel_path
        )
